...
import { AppState, DEFAULT_STATE, DrawRecord, Plan, Score, BASE_POOL, PLAN_PRICE } from "./types";
...
export function generateTicket(scores: Score[]): number[] | null {
  ...
  const seedStr = scores.map((s) => `${s.date}:${s.points}`).sort().join("|");
...
export function useAppState() {
  ...
  const completeOnboarding = useCallback((plan: Plan, charityId: string, charityPct: number) => {
  ...
  const addScore = useCallback((date: string, points: number): string | null => {
    let error: string | null = null;
    setState((s) => {
      if (s.scores.some((sc) => sc.date === date)) {
        error = "You already logged a round for this date — edit it below instead.";
        return s;
      }
      let scores = [...s.scores, { id: "s" + Date.now(), date, points }];
      if (scores.length > 5) {
  ...
  const editScore = useCallback((id: string, date: string, points: number): string | null => {
    let error: string | null = null;
    setState((s) => {
      if (s.scores.some((sc) => sc.date === date && sc.id !== id)) {
        error = "Another round is already logged for that date.";
        return s;
      }
      return {
        ...s,
        scores: s.scores.map((sc) => (sc.id === id ? { ...sc, date, points } : sc)),
      };
    });
    return error;
  }, []);
  ...
  const deleteScore = useCallback((id: string) => {
    setState((s) => ({ ...s, scores: s.scores.filter((sc) => sc.id !== id) }));
  }, []);

  const setCharity = useCallback((charityId: string) => {
    setState((s) => ({ ...s, charityId }));
  }, []);

  const setCharityPct = useCallback((charityPct: number) => {
    setState((s) => ({ ...s, charityPct }));
  }, []);

  const addDonation = useCallback((amount: number) => {
    setState((s) => ({ ...s, independentTotal: s.independentTotal + amount }));
  }, []);
  ...
  const runDraw = useCallback((): DrawRecord | null => {
    ...
    setState((s) => {
      ...
      const matches = ticket.filter((n) => result.includes(n)).length;

      const tier5 = BASE_POOL * 0.4 + s.jackpotRollover;
      ...
      return { ...s, draws: [...s.draws, record], jackpotRollover };
    });
    return record;
  }, []);

  const advanceDrawStatus = useCallback((id: string, next: DrawRecord["status"]) => {
    setState((s) => ({
      ...s,
      draws: s.draws.map((d) => (d.id === id ? { ...d, status: next } : d)),
    }));
  }, []);
