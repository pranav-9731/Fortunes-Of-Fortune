...
function drawWeighted(users: AdminUser[]): number[] {
  ...
  while (picked.size < 5) {
    picked.add(1 + Math.floor(Math.random() * 49));
  }
...
function computeTiers(
  ...
  const byTier: Record<3 | 4 | 5, string[]> = {
    3: [],
    4: [],
    5: [],
  };
  ...
  return [
    {
      ...
      poolShare: tier5Pool,
      winnerIds: byTier[5],
      amountEach: byTier[5].length
        ? tier5Pool / byTier[5].length
        : 0,
    },
    {
      tier: 4,
      poolShare: tier4Pool,
      winnerIds: byTier[4],
      amountEach: byTier[4].length
        ? tier4Pool / byTier[4].length
        : 0,
    },
    {
      tier: 3,
      poolShare: tier3Pool,
      winnerIds: byTier[3],
      amountEach: byTier[3].length
        ? tier3Pool / byTier[3].length
        : 0,
    },
  ];
}
...
export function useAdminState() {
  ...
  useEffect(() => {
    if (hydrated) saveJSON(USERS_KEY, users);
  }, [users, hydrated]);

  useEffect(() => {
    if (hydrated) saveJSON(DRAWS_KEY, draws);
  }, [draws, hydrated]);

  useEffect(() => {
    if (hydrated) saveJSON(DRAFT_KEY, draft);
  }, [draft, hydrated]);

  useEffect(() => {
    if (hydrated) saveJSON(WINNERS_KEY, winners);
  }, [winners, hydrated]);

  const activeUsers = users.filter((u) => u.status === "active");

  const totalPool = activeUsers.reduce(
    (sum, u) => sum + u.planPrice * POOL_FUNDING_RATE,
    0
  );
  ...
  const editUserProfile = useCallback(
    (id: string, fields: Partial<AdminUser>) => {
      setUsers((list) =>
        list.map((u) => (u.id === id ? { ...u, ...fields } : u))
      );
    },
    []
  );
  ...
  const editUserScore = useCallback(
    (userId: string, scoreId: string, date: string, points: number) => {
      setUsers((list) =>
        list.map((u) =>
          u.id === userId
            ? {
      ...
      setUsers((list) =>
        list.map((u) =>
          u.id === userId
            ? {
                ...u,
                scores: u.scores.filter((s) => s.id !== scoreId),
              }
            : u
        )
      );
    ...
    (userId: string, date: string, points: number) => {
      setUsers((list) =>
        list.map((u) => {
          ...
          if (u.scores.some((s) => s.date === date)) {
            return u;
          }
  ...
  const publishResults = useCallback(() => {
    if (
      draft.stage !== "simulated" ||
      !draft.simulatedNumbers ||
      !draft.simulatedTiers
    ) {
      return;
    }
